...
###### sum/average expression of NF-kB targetd genes in monocyte/neutrophil
...
# sum expression of pathway "NETs" genes
# Extract the normalized expression values
norm_expr <- GetAssayData(merged_seurat_FilteredCells_Pdlim2Active_neutrophil, layer = "data")

# sum expression for your gene set per cell
# IPA GSE243629 pathway "NETs" DEGs
gene_set<- c("BPI", "C5", "C1QA", "C1QC", "CASP1", "CASP3", "CASP5", "CCL5", "CCR1", "COL15A1", "DEFA3", "DEFA4", "HCK", "IGHG2", "MAPK8", "MAPK14", "PIK3CB", "PIK3CD", "PLA2G4A", "PLAAT3", "PLAAT4", "PLCB2", "PLCD1", "RIPK3", "SIGLEC5", "TIMM13", "TIMM23", "TIMM8A", "C1QB", "CASP10", "CXCL8", "HIF1A", "ITGB2", "LAIR1", "MAP2K6", "MT-CYB", "MT-ND3", "PRKCE", "RAC1", "SYK", "TIMM10", "TSPO", "VSTM1", "CASP6", "COL9A2", "DOCK1", "ITPR1", "ITPR2", "MT-ATP6", "PNPLA2", "PPP3CA", "PRKCZ", "SIGLEC9")
...
# add them to metadata
merged_seurat_FilteredCells_Pdlim2Active_neutrophil$NETs <- gene_set_sum

colnames(merged_seurat_FilteredCells_Pdlim2Active_neutrophil[[]])
head(merged_seurat_FilteredCells_Pdlim2Active_neutrophil[[]]$NETs)

# fetch data
merged_seurat_FilteredCells_Pdlim2Active_neutrophil_NETs <- FetchData(object = merged_seurat_FilteredCells_Pdlim2Active_neutrophil, vars = c("sample", "expression_group", "PDLIM2", "NETs")) 
write.csv (merged_seurat_FilteredCells_Pdlim2Active_neutrophil_NETs, file="merged_seurat_FilteredCells_Pdlim2Active_neutrophil_NETs-PDLIM2.csv")
...
# Ribavirin ADME
# sum expression of Ribavirin ADME genes in neutrophil
# Extract the normalized expression values
norm_expr <- GetAssayData(merged_seurat_FilteredCells_Pdlim2Active_neutrophil, layer = "data")

# IPA GSE243629_neutrophil_Pdlim2High-Ctrl Ribavirin ADME DEGs
gene_set <- c("NME7", "NT5C2", "PI4KB", "SLC28A3", "SPHk1", "STK10", "STK24")
...
# add them to metadata
merged_seurat_FilteredCells_Pdlim2Active_neutrophil$Ribavirin <- gene_set_sum

colnames(merged_seurat_FilteredCells_Pdlim2Active_neutrophil[[]])
head(merged_seurat_FilteredCells_Pdlim2Active_neutrophil[[]]$Ribavirin)

# fetch data
merged_seurat_FilteredCells_Pdlim2Active_neutrophil_Ribavirin <- FetchData(object = merged_seurat_FilteredCells_Pdlim2Active_neutrophil, vars = c("sample", "expression_group", "PDLIM2", "Ribavirin")) 
write.csv (merged_seurat_FilteredCells_Pdlim2Active_neutrophil_Ribavirin, file="GSE243629_Pdlim2Active_Monocyte_Ribavirin-PDLIM2.csv")

# sum expression of reactome IL-10 genes in neutrophil
# Extract the normalized expression values
norm_expr <- GetAssayData(merged_seurat_FilteredCells_Pdlim2Active_neutrophil, layer = "data")

#reactome IL-10 molecules
gene_set <- c("CCL19","CCL2","CCL20","CCL22","CCL3","CCL3L3","CCL4","CCL5","CCR1","CCR2","CCR5","CD80","CD86","CSF1","CSF2","CSF3","CXCL1","CXCL10","CXCL2","CXCL8","FCER2","FPR1","ICAM1","IL10","IL10RA","IL10RB","IL12A","IL12B","IL18","IL1A","IL1B","IL1R1","IL1R2","IL1RN","IL6","JAK1","LIF","PTAFR","PTGS2","STAT3","TIMP1","TNF","TNFRSF1A","TNFRSF1B","TYK2")
...
# add them to metadata
merged_seurat_FilteredCells_Pdlim2Active_neutrophil$IL10 <- gene_set_sum

colnames(merged_seurat_FilteredCells_Pdlim2Active_neutrophil[[]])
head(merged_seurat_FilteredCells_Pdlim2Active_neutrophil[[]]$IL10)

# fetch data
merged_seurat_FilteredCells_Pdlim2Active_neutrophil_IL10 <- FetchData(object = merged_seurat_FilteredCells_Pdlim2Active_neutrophil, vars = c("sample", "expression_group", "PDLIM2", "IL10")) 
write.csv (merged_seurat_FilteredCells_Pdlim2Active_neutrophil_IL10, file="GSE243629_Pdlim2Active_Monocyte_IL10-PDLIM2.csv")


# sum expression of pathway MHC class II antigen presentation genes
# Extract the normalized expression values
norm_expr <- GetAssayData(merged_seurat_FilteredCells_Pdlim2Active_Monocyte, layer = "data")

# Reactome pathway MHC class II antigen presentation molecules
gene_set <- c("TUBAL3", "TUBB8B", "KIF2A", "DCTN6", "DYNC1I1", "KIF3C", "SPTBN2", "KIF3B", "DYNC1LI2", "AP1G1", "CTSV", "DCTN3", "SEC24D", "AP2A2", "SEC31A", "KIF20A", "KIF4A", "SEC24A", "SEC24B", "AP2A1", "HLA-DRA", "HLA-DQA2", "HLA-DQA1", "HLA-DRB1", "HLA-DQB1", "CD74", "TUBB4A", "HLA-DPB1", "HLA-DQB2", "HLA-DOA", "CTSD", "CTSL", "CTSB", "CLTA", "CTSH", "TUBA3C", "TUBA3D", "CTSA", "IFI30", "HLA-DRB4", "HLA-DOB", "CTSE", "LAG3", "HLA-DPA1", "CTSS", "CANX", "HLA-DMA", "HLA-DMB", "KIF5B", "ACTR1B", "CTSO", "CTSK", "CAPZA2", "CAPZB", "DNM2", "RAB7A", "KIF11", "CAPZA1", "CTSC", "AP2S1", "SEC24C", "SEC13", "AP1S2", "ACTR1A", "AP1S1", "AP2B1", "DYNLL1", "TUBA1B", "TUBA4A", "TUBB4B", "HLA-DRB3", "ARF1", "CLTC", "CENPE", "KIF23", "DNM1", "KLC1", "AP1B1", "KIF5A", "DYNC1I2", "TUBB3", "DCTN2", "TUBB2A", "DCTN1", "DYNC1H1", "KIF22", "SEC23A", "KIF4B", "HLA-DRB5", "TUBB8", "", "KLC3", "TUBA3E", "", "TUBA1A", "KIF2B", "KIF18A", "KIFAP3", "AP2M1", "DYNLL2", "CAPZA3", "RILP", "AP1S3", "LGMN", "KIF2C", "SH3GL2", "TUBA1C", "DCTN5", "TUBB6", "TUBB2B", "AP1M1", "OSBPL1A", "KLC2", "RACGAP1", "TUBB1", "TUBA4B", "KIF15", "KLC4", "TUBA8", "ACTR10", "CTSF", "DCTN4", "KIF26A", "DNM3", "KIF3A", "SAR1B", "DYNC1LI1", "AP1M2")
...
# add them to metadata
merged_seurat_FilteredCells_Pdlim2Active_Monocyte$MHCII <- gene_set_sum

colnames(merged_seurat_FilteredCells_Pdlim2Active_Monocyte[[]])
head(merged_seurat_FilteredCells_Pdlim2Active_Monocyte[[]]$MHCII)

# fetch data
merged_seurat_FilteredCells_Pdlim2Active_Monocyte_MHCII <- FetchData(object = merged_seurat_FilteredCells_Pdlim2Active_Monocyte, vars = c("sample", "expression_group", "PDLIM2", "MHCII")) 
write.csv (merged_seurat_FilteredCells_Pdlim2Active_Monocyte_MHCII, file="merged_seurat_FilteredCells_Pdlim2Active_Monocyte_MHCII-PDLIM2.csv")

# sum expression of pathway immunogenic cell death genes
# Extract the normalized expression values
norm_expr <- GetAssayData(merged_seurat_FilteredCells_Pdlim2Active_Monocyte, layer = "data")

# Reactome pathway immunogenic cell death molecules
# Reactome programmed cell death molecules
gene_set <- c("ARHGAP10", "TLR4", "TNFRSF10A", "PSMD11", "PSMD12", "DFFA", "DNM1L", "PSMD14", "SDCBP", "APAF1", "TNFRSF10B", "PSMA7", "OGT", "TP73", "CFLAR", "SEPTIN4", "PSMD3", "DAPK3", "BCL2L11", "CHMP2A", "GAS2", "PRKN", "OPA1", "GSDME", "FLOT1", "DFFB", "IL1A", "IL1B", "LMNA", "", "TP53", "GSN", "H1-0", "HSP90AA1", "ELANE", "", "CD14", "VIM", "HMGB1", "UBB", "UBC", "GZMB", "H1-4", "BCL2", "MAPT", "IRF1", "CDH1", "IRF2", "DSP", "H1-5", "H1-3", "H1-2", "PSMC3", "PSMB1", "LMNB1", "APC", "FAS", "PSMA1", "PSMA2", "PSMA3", "PSMA4", "HMGB2", "YWHAQ", "MAPK3", "PSMA5", "PSMB4", "PSMB6", "PSMB5", "MAPK1", "CASP1", "NMT1", "AKT1", "AKT2", "YWHAB", "SFN", "DSG3", "CTNNB1", "ADD1", "PSMC2", "STAT3", "CASP3", "DCC", "PSMC4", "MAPK8", "FASLG", "PPP3CC", "PSMD8", "FNTA", "CASP4", "PSMB3", "PSMB2", "TNFSF10", "BCAP31", "PSMD7", "BMX", "CASP5", "KPNA1", "DAPK1", "CASP7", "CASP9", "CASP6", "BID", "GSDMD", "SEM1", "PSMA6", "YWHAG", "PSMC1", "PSMC5", "YWHAE", "PSMC6", "RPS27A", "UBA52", "PPP3R1", "YWHAZ", "DYNLL1", "UBE2L3", "XIAP", "CYCS", "E2F1", "SATB1", "DSG1", "H1-1", "PRKCQ", "YWHAH", "PTK2", "PRKCD", "C1QBP", "TJP1", "BAX", "BCL2L1", "TRAF2", "FADD", "PAK2", "PSMD2", "ROCK1", "BIRC3", "BIRC2", "RIPK1", "TP53BP2", "PMAIP1", "SPTAN1", "PKP1", "IL18", "DSG2", "TFDP1", "TFDP2", "FLOT2", "CASP8", "KPNB1", "PSMD6", "PLEC", "TRADD", "ADRM1", "CDC37", "BAK1", "OCLN", "UNC5A", "", "TMED7-TICAM2", "TICAM1", "UNC5B", "CDKN2A", "MLKL", "PDCD6IP", "CHMP7", "BAD", "CHMP4C", "OMA1", "PELI1", "DYNLL2", "CHMP6", "APIP", "ITCH", "PPP1R13B", "BMF", "", "PSMB7", "PSMD1", "BBC3", "CHMP4A", "UACA", "TP63", "CHMP4B", "CLSPN", "AVEN", "DIABLO", "STK26", "TJP2", "DAPK2", "DBNL", "APPL1", "ACIN1", "STUB1", "PSMD13", "CHMP2B", "AKT3", "CARD8", "CHMP3", "RIPK3", "MAGED1", "STK24", "LY96")
...
# add them to metadata
merged_seurat_FilteredCells_Pdlim2Active_Monocyte$ImmunogenicCellDeath <- gene_set_sum

colnames(merged_seurat_FilteredCells_Pdlim2Active_Monocyte[[]])
head(merged_seurat_FilteredCells_Pdlim2Active_Monocyte[[]]$ImmunogenicCellDeath)

# fetch data
merged_seurat_FilteredCells_Pdlim2Active_Monocyte_ImmunogenicCellDeath <- FetchData(object = merged_seurat_FilteredCells_Pdlim2Active_Monocyte, vars = c("sample", "expression_group", "PDLIM2", "ImmunogenicCellDeath")) 
write.csv (merged_seurat_FilteredCells_Pdlim2Active_Monocyte_ImmunogenicCellDeath, file="merged_seurat_FilteredCells_Pdlim2Active_Monocyte_ImmunogenicCellDeath-PDLIM2.csv")

# sum expression of reactome oxidative stress induced senescence molecules
# Extract the normalized expression values
norm_expr <- GetAssayData(merged_seurat_FilteredCells_Pdlim2Active_Monocyte, layer = "data")
